...
type URLRepository interface {
	...
	// GetByOriginalURL получает оригинальный URL по короткой ссылке
	GetByOriginalURL(ctx context.Context, originalURL string) (*models.URLPair, error)

	// List получает список всех URL пользователя
	ListByUser(ctx context.Context, userID string) ([]*models.URLPair, error)
